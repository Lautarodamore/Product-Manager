      ...
      <v-row justify="center" wrap>
        <v-col cols="12" md="6" xs="12">
          <h1 style="text-align:center; color:#98989C">
            Administrador de Tipo de Productos
          </h1>
        </v-col>
      </v-row>
          ...
          >
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-toolbar-title>Tipo de Productos</v-toolbar-title>
                <v-divider class="mx-4" inset vertical></v-divider>
                <v-spacer></v-spacer>
                <v-dialog v-model="dialog" max-width="500px">
                  <template v-slot:activator="{ on }">
                    <v-btn color="primary" dark class="mb-2" v-on="on"
                      >Nuevo Tipo de Producto</v-btn
                    >
                  </template>
                  <v-card>
                    <v-card-title>
  ...
  methods: {
    async initialize() {
      const response = await axios.get(
        "http://localhost:3000/api/v1/type-product"
      );
    ...
    async deleteItem(item) {
      ...
      confirm("Estas seguro que quieres elimar este tipo de producto?") &&
        ...
        (await axios.delete(
          `http://localhost:3000/api/v1/type-product/${item.id}`
        ));
    },
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        await axios.patch(
          `http://localhost:3000/api/v1/type-product/${this.editedItem.id}`,
          this.editedItem
        );
      } else {
